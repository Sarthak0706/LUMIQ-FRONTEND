...
const PolicyForm = ({ userId, onPolicyCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Normalize policyType to lowercase and check for variations of 'pending'
    const normalizedPolicyType = policyType.trim().toLowerCase();

    if (normalizedPolicyType === "pending") {
      setError("Policy type cannot be 'Pending' in any form.");
      return;
    }

    const policyData = {
      policyNumber,
      policyType: policyType, // Store original policy type value
      amount, // Add amount to the policy data
      userId, // Attach userId to link policy with user
    };
